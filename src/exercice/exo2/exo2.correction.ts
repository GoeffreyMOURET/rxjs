import { catchError, concatMap, filter, iif, map, mergeMap, Observable, of } from "rxjs";
 ...
 */
export default class ExerciceCorrection2 {
    ...
    transmettreInfoUtilisateurDeNotificationErreur(): Observable<UtilisateurModele> {
        return this.serviceWeb.recupererNotifications().pipe(
            filter((notification) => !!notification.idUtilisateur && notification.type === 'ERROR'),
            mergeMap((notification) => this.serviceWeb.recupererUtilisateur(notification.idUtilisateur!))
        );
    }
    ...
    transmettreErreursEtWarningsEnrichis(): Observable<NotificationModele & { role ?: Role, }> {
        return this.serviceWeb.recupererNotifications().pipe(
            filter((notification) => notification.type === 'ERROR' || notification.type === 'WARNING'),
            concatMap((notification) => iif(
                () => !!notification.idUtilisateur,
                this.serviceWeb.recupererUtilisateur(notification.idUtilisateur!).pipe(
                    map((utilisateur) => ({ ...notification, role: utilisateur.role, }))
                ),
                of(notification)
            ))
        )
            ...
            concatMap((notification) => iif(
                ...
                this.serviceWeb.recupererUtilisateur(notification.idUtilisateur!).pipe(
                    ...
                    catchError((err: unknown) => {
                        ...
                        return of({ ...notification, role: 'INCONNU' as Role, });
                    })
                ),
                of(notification)
            ))
